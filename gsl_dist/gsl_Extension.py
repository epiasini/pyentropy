...
class gsl_Extension(Extension):
        ...
        def __init__(self, name, sources,
            ...
            gsl_lib_list=list(map(str.strip,self.get_gsl_libs().split()))

            # filter options with -l
            not_lib_opt=lambda a:a[:2]=="-l"
            gsl_lib_list=list(filter(not_lib_opt,gsl_lib_list))
            # cut -l
            only_lib_name=lambda a:a[2:]
            gsl_lib_list=list(map(only_lib_name,gsl_lib_list))
            ...
            tmp = [x[0] for x in define_macros]
